...
def get_slide_level(slide, target_magnification=20, target_mpp=0.5):
    """
    Get the appropriate level for a given microns per pixel (mpp) and magnification.
    
    Args:
      slide: OpenSlide object.
      target_mpp: The target microns per pixel (e.g., 0.5).
      target_magnification: The target magnification (e.g., 20X).
    
    Returns:
      The appropriate level index.
    """
    level_count = slide.level_count
    level_downsamples = slide.level_downsamples
    
    # Calculate base mpp from slide properties
    base_mpp_x = float(slide.properties.get('openslide.mpp-x', 0))
    base_mpp_y = float(slide.properties.get('openslide.mpp-y', 0))
    
    if base_mpp_x == 0 or base_mpp_y == 0:
        warnings.warn("Base MPP not found in slide properties.")
        return 0
    
    base_mpp = (base_mpp_x + base_mpp_y) / 2.0
    
    # Calculate the mpp for each level
    mpps = [base_mpp * downsample for downsample in level_downsamples]
    
    # print("Level count:", level_count)
    # print("Level downsamples:", level_downsamples)
    # print("Base MPP:", base_mpp)
    # print("Calculated MPPs for each level:", mpps)
    
    # Find the levels with the closest mpp to the target mpp
    closest_mpp_levels = sorted(range(level_count), key=lambda i: abs(mpps[i] - target_mpp))
    closest_mpp = abs(mpps[closest_mpp_levels[0]] - target_mpp)
    
    # Filter levels that are within the closest mpp threshold
    filtered_levels = [level for level in closest_mpp_levels if abs(mpps[level] - target_mpp) == closest_mpp]
    
    # Retrieve objective power
    objective_power = float(slide.properties.get('openslide.objective-power', 0))
    
    if objective_power == 0:
        warnings.warn("Objective power not found in slide properties.")
        return 0
    
    # If there are multiple levels with the same closest mpp, find the one with the closest magnification
    if len(filtered_levels) > 1:
        best_level = min(filtered_levels, key=lambda level: abs(objective_power / level_downsamples[level] - target_magnification))
    else:
        best_level = filtered_levels[0]
    
    # print("Selected level:", best_level)
    # print("MPP of selected level:", mpps[best_level])
    # print("Magnification of selected level:", objective_power / level_downsamples[best_level])
    
    return best_level

def tile_to_pil_tile(tile, preload_slide=None, auto_level=False):
    """
...
def tile_to_np_tile(tile, preload_slide=None, auto_level=False):
    """
    ...
    """
    pil_img = tile_to_pil_tile(tile, preload_slide, auto_level=auto_level)
...
def save_display_tile(tile, label_type, _env_base_train_dir,
                      preload_slide=None, save=True, display=False, auto_level=False):
    """
    ...
    """
    tile_pil_img = tile_to_pil_tile(tile, preload_slide, auto_level=auto_level)
...
class Tile:
    ...
    def get_pil_tile(self, preload_slide=None):
        ''' !!! only treat auto_level for TCGA datasets'''
        if self.original_slide_filepath.find('TCGA') != -1:
            auto_level = True
        else:
            auto_level = False 
        return tile_to_pil_tile(self, preload_slide, auto_level=auto_level)
    ...
    def get_np_tile(self, preload_slide=None):
        ...
        if self.original_slide_filepath.find('TCGA') != -1:
            ...
            auto_level = False
        return tile_to_np_tile(self, preload_slide, auto_level=auto_level)
    ...
    def change_root(self, root_1, root_2, change_sep=True):
        '''
        change the file_path from server_root to pc_root (or reverse and even mac_root.)
        used for sometime, apply the model learned from server but would like to be tested on PC
        '''
        self.original_slide_filepath = self.original_slide_filepath.replace(root_1, root_2)
        if change_sep:
            self.original_slide_filepath = self.original_slide_filepath.replace(f'{os.sep}', '/')
