...
def filter_black(rgb, upper_thresh, output_type="bool"):
    """
    Create a mask to filter out blackish colors, where the mask is based on a pixel being below an
    upper threshold value in the red, green, and blue channels.
    
    Args:
      rgb: RGB image as a NumPy array.
      upper_thresh: Upper threshold value for all channels.
      output_type: Type of array to return (bool, float, or uint8).
    ...
    r = rgb[:,:, 0] < upper_thresh
    g = rgb[:,:, 1] < upper_thresh
    b = rgb[:,:, 2] < upper_thresh
    result = ~(r & g & b)
    if output_type == "bool":
        pass
    elif output_type == "float":
...
def filter_black_pen(rgb, output_type="bool", show_np_info=False):
    """
    ...
    Returns:
      NumPy array representing the mask.
    """
    t = Time()  # Assuming Time() is defined somewhere in your code
    result = filter_black(rgb, upper_thresh=150)  # Example threshold
    if output_type == "bool":
        pass
    elif output_type == "float":
        result = result.astype(float)
        ...
        result = result.astype("uint8") * 255
    if show_np_info == True:
        tools.np_info(result, "Filter Black Pen", t.elapsed())
...
#     rgb_no_blue_pen = image_tools.mask_rgb(np_rgb, mask_no_blue_pen)
    ...
    mask_no_black_pen = filter_black_pen(np_rgb)
